...
for sid in sids :
    ...
    # HDFS 경로 설정
    hdfs_path = f'/article/article_data_100p_{today_date}_{sid}.json'

    # HDFS 클라이언트 생성
    client = hdfs.InsecureClient('http://localhost:9870', user='dev')

    # HDFS에 파일 업로드
    client.upload(hdfs_path, txt_file_path)

subprocess.run(["python3", hadoop_test01.py])
